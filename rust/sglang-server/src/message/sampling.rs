...
mod tests {
    ...
    #[test]
    fn direct_preferred_merge_preserves_stop_normalization() {
        let preferred = serde_json::json!({
            "stop": ["END", "STOP"],
            "stop_regex": "\\d{3}"
        });
        let mut input: SamplingParamsInput = serde_json::from_str("{}").unwrap();
        input.apply_preferred(&preferred).unwrap();
        let SamplingParamsInput::One(mut params) = input else {
            unreachable!()
        };

        params.normalize(false, TEST_VOCAB).unwrap();

        assert_eq!(params.stop, None);
        assert_eq!(params.stop_regex, None);
        assert_eq!(params.stop_strs, ["END", "STOP"]);
        assert_eq!(params.stop_regex_strs, [r"\d{3}"]);
        assert_eq!(params.stop_str_max_len, 4);
        assert_eq!(params.stop_regex_max_len, 3);
    }
}
